...
$scope.uploadFile = function(processId,parentProcessId,subDir,cg) {
    ...
    if (dataRecord ) {
         ...
         cfgDetails = propertiesAC('/mdrest/properties/', 'PUT', putData);
             ...
             console.log(cg);
             var key=cg+'Path';
             var property = propertiesAC('/mdrest/properties/', 'DELETE', [processId,key]);
             var putData = "configGroup="+cg+"&key="+key+"&value="+subDir+'/'+dataRecord.Record.fileName+"&description="+'File path'+"&processId="+processId;
             cfgDetails = propertiesAC('/mdrest/properties/', 'PUT', putData);
            $.get('/mdrest/properties/'+processId, function(getdata) {
                $scope.chartViewModel.selectedProcessProps = getdata.Record;
            });
