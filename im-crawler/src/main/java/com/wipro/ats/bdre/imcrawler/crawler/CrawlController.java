...
//  protected final Environment env;
    ...
    public CrawlController(CrawlConfig config, PageFetcher pageFetcher, RobotstxtServer robotstxtServer, int pid, long instanceExecid)
            throws Exception {
        super(config);
...
//    env = new Environment(envHome, envConfig);
        ...
        frontier = new Frontier(config, pid, instanceExecid);
        ...
        finished = false;
        shuttingDown = false;
        CrawlRecordReader.shuttingDown = false;
    }
                            ...
                            while (true) {
                                ...
                                for (int i = 0; i < threads.size(); i++) {
                                    ...
                                    if (!thread.isAlive()) {
                                        ...
                                        //check to see that some thread working or not
...
//                    env.close();
    ...
    public void addSeed(String pageUrl, int docId) {
        ...
        if (canonicalUrl == null) {
            ...
            if (docId < 0) {  //Docid is -1 so need to auto generate it; that is done by DB
            ...
            } else { //its getting the docid so set pass it to DB
                try {
    ...
    public void shutdown() {
        ...
        CrawlRecordReader.shuttingDown = true;
        pageFetcher.shutDown();
        frontier.finish();
    }
}
