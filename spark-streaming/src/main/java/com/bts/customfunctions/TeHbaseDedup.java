 ...
 */
public class TeHbaseDedup  extends Custom {
    ...
    public JavaPairDStream<String, WrapperMessage> convertJavaPairDstream(JavaPairDStream<String, WrapperMessage> inputDstream, Map<String, Broadcast<HashMap<String, String>>> broadcastMap, JavaStreamingContext jssc) {
        ...
        JavaPairDStream<String, Integer> existingDataInResolvedHBase =
                buskeyStream.transform(
                        new BulkGetRowKeyByKey(getHBaseContext(jssc.sparkContext()), "resolved"))
...
class TransactionElementBusinessKeyAttribution2 implements Serializable,PairFunction<Tuple2<String, Row>, String, WrapperMessage> {
    @Override
...
class TransactionElementHbaseUtils {
...
class TransactionElementBulkGetRowKeyByKey implements Function<JavaRDD<String>, JavaRDD<String>> {


    private JavaHBaseContext hbaseContext;
    private String tableName;

    public TransactionElementBulkGetRowKeyByKey(JavaHBaseContext hbaseContext, String tableName) {
...
class TransactionElementBulkGetRowKeyFromUnresolved implements Function<JavaRDD<String>, JavaRDD<String>> {


    private JavaHBaseContext hbaseContext;
    private String tableName;

    public TransactionElementBulkGetRowKeyFromUnresolved(JavaHBaseContext hbaseContext, String tableName) {
        this.hbaseContext = hbaseContext;
        this.tableName = tableName;
    }
    ...
    public JavaRDD<String> call(JavaRDD<String> keys) throws Exception {
        return hbaseContext.bulkGet(TableName.valueOf(tableName), 2, keys,
                new TransactionElementRowKeyGetFunction(),
                new TransactionElementRowKeyResultFunctionUnresolved()).filter(key -> (key != null));
    }
}

class TransactionElementRowKeyGetFunction implements Function<String, Get> {
...
class TransactionElementRowKeyResultFunctionUnresolved implements Function<Result, String> {

    public String call(Result result) throws Exception {
        String dealTuple = Bytes.toString(result.getValue("transactionelements".getBytes(),"event".getBytes()));
        if(dealTuple != null){
