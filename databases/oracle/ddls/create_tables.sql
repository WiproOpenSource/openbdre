...
CREATE TABLE INSTALLED_PLUGINS (
  ...
  VERSION number(11,0) NOT NULL DEFAULT 1,
  AUTHOR VARCHAR(128),
  ADD_TS TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PLUGIN VARCHAR(128),
  UNINSTALLABLE number(0,1) DEFAULT 1,
  PRIMARY KEY (PLUGIN_UNIQUE_ID)
) ;


CREATE TABLE PLUGIN_DEPENDENCY (
  DEPENDENCY_ID number(11,0) NOT NULL ,
  PLUGIN_UNIQUE_ID VARCHAR(128) NOT NULL,
  DEPENDENT_PLUGIN_UNIQUE_ID VARCHAR(128),
  PRIMARY KEY (DEPENDENCY_ID),
  CONSTRAINT PLUGIN_UNIQUE_ID_fk FOREIGN KEY (PLUGIN_UNIQUE_ID) REFERENCES INSTALLED_PLUGINS (PLUGIN_UNIQUE_ID) enable,
  CONSTRAINT DEPENDENT_PLUGIN_UNIQUE_ID_fk FOREIGN KEY (DEPENDENT_PLUGIN_UNIQUE_ID) REFERENCES INSTALLED_PLUGINS (PLUGIN_UNIQUE_ID) enable
) ;


CREATE SEQUENCE PLUGIN_DEPENDENCY_SEQ
       MINVALUE 1
       MAXVALUE 9999999999999999999
       START WITH 1
       INCREMENT BY 1
...
CREATE TABLE PLUGIN_CONFIG (
  ...
  PLUGIN_KEY number(11,0),
  PLUGIN_VALUE VARCHAR(128),
  PRIMARY KEY (PLUGIN_KEY,PLUGIN_UNIQUE_ID),
  CONSTRAINT PLUGIN_CONFIG_ID_fk FOREIGN KEY (PLUGIN_UNIQUE_ID) REFERENCES INSTALLED_PLUGINS (PLUGIN_UNIQUE_ID) enable
) ;
