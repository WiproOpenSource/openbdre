...
CREATE TABLE PROCESS (
  ...
  PARENT_PROCESS_ID INT(11) DEFAULT NULL,
  CAN_RECOVER TINYINT(1) DEFAULT '1',
  ENQUEUING_PROCESS_ID VARCHAR(255) NOT NULL DEFAULT '0',
  BATCH_CUT_PATTERN VARCHAR(45) DEFAULT NULL,
...
CREATE TABLE PROCESS_EXECUTION_QUEUE (
   ...
   BUS_DOMAIN_ID INT(11) NOT NULL,
   PROCESS_TYPE_ID INT(11) NOT NULL,
   EXEC_SCRIPT_LOCATION VARCHAR(1000) DEFAULT NULL,
   PRIMARY KEY (EXECUTION_ID),
   KEY EXEC_STATUS_ID (EXEC_STATUS_ID),
   KEY EXEC_PROCESS_ID (PROCESS_ID),
   KEY EXEC_PROCESS_TYPE_ID (PROCESS_TYPE_ID),
   KEY EXEC_BUS_DOMAIN_ID (BUS_DOMAIN_ID),
   CONSTRAINT EXEC_STATUS_ID FOREIGN KEY (EXEC_STATUS_ID) REFERENCES EXEC_STATUS (EXEC_STATE_ID) ON DELETE NO ACTION ON UPDATE NO ACTION,
   CONSTRAINT EXEC_PROCESS_ID FOREIGN KEY (PROCESS_ID) REFERENCES PROCESS (PROCESS_ID) ON DELETE NO ACTION ON UPDATE NO ACTION,
   CONSTRAINT EXEC_PROCESS_TYPE_ID FOREIGN KEY (PROCESS_TYPE_ID) REFERENCES PROCESS_TYPE (PROCESS_TYPE_ID) ON DELETE NO ACTION ON UPDATE NO ACTION,
   CONSTRAINT EXEC_BUS_DOMAIN_ID FOREIGN KEY (BUS_DOMAIN_ID) REFERENCES BUS_DOMAIN (BUS_DOMAIN_ID) ON DELETE NO ACTION ON UPDATE NO ACTION
)  ENGINE=INNODB DEFAULT CHARSET=UTF8;

CREATE TABLE ACCOUNT_LOCKOUT(
    USERNAME VARCHAR (45) NOT NULL,
    INSERT_TS TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    LOCK_STATUS VARCHAR (45) NOT NULL,
    ATTEMPTS INT(11) ,
    PRIMARY KEY (USERNAME)
) ENGINE=INNODB DEFAULT CHARSET=UTF8;
